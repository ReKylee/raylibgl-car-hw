...
namespace raylibgl::model {
    ...
    namespace {
        ...
        /// @brief One baked Blockbench cuboid: centre + size, an axis-angle rotation (degrees) and a flat colour.
        struct Part {
            float center[3];
            float size[3];
        ...
        // Side decal plane, flush on the side wall so it can't float or clip.
        ...
        /// @brief Lazily load the side-decal texture and the radial glow sprite (once).
        ...
        /// @brief Draw a baked group of cuboids, each with its own translate + rotate.
        ...
        /**
         * @brief Fan-triangulate one convex profile piece into both side walls (outward normals).
         * @param p Convex profile (z,y) points, triangulated as a fan from p[0].
         * @param L Left side-wall x (normal -X).
         * @param R Right side-wall x (normal +X).
         */
        ...
        /// @brief Draw the body as one extruded-hull mesh: bumpers, belly pan, tiled side walls,
        ///        swept outline faces (skipping the windshield rake) and the inset doors.
        ...
        /// @brief Draw a general quad (filled or wire) with an explicit outward normal @p nrm.
        ...
        /**
         * @brief Draw one front door's body-colour frame and its recess reveals.
         *
         * An equal-width frame panel recessed at DOOR_X, with reveal walls closing the door
         * recess (body -> panel) and the window recess (panel -> glass). The glass itself is
         * drawn by drawDoorGlass.
         * @param sx Side: -1 = left, +1 = right.
         */
        void drawDoorFrame(int sx, bool wire) {
        ...
        /// @brief Draw one front-door's semi-transparent glass trapezoid, recessed at GLASS_X (glass pass).
        /// @param sx Side: -1 = left, +1 = right.
        ...
        /// @brief Draw the cabin's front bench: a flat cushion box plus a seat-back box tilted
        ///        back, solid so it reads in wireframe too.
        ...
        /// @brief Emit one triangle with a computed outward face normal (inside rlBegin(RL_TRIANGLES)).
        ...
        /**
         * @brief Draw a UV sphere band between latitudes @p latMin and @p latMax.
         *
         * A full sphere spans -PI/2..PI/2; a smaller @p latMax leaves an open top (a bowl).
         * Emits outward per-vertex normals; the wire path draws a latitude/longitude cage.
         */
        ...
        /// @brief Draw a flat horizontal disk (normal +Y), used for the aquarium water surface.
        ...
        /**
         * @brief Draw the bench-top fishbowl aquarium.
         *
         * Three layers sell "water in glass": an inner water dome capped by a flat surface
         * disk, inside a near-transparent glass bowl cut open flat at the top. Drawn in the
         * glass pass with depth-write and culling off so the translucent layers blend cleanly.
         */
        ...
        constexpr float FISH_LENGTH = AQ_RADIUS * 0.5f;  // total length = half the aquarium radius
        constexpr Vector3 FISH_CENTER{AQ_CENTER.x, AQ_CENTER.y - 0.05f, AQ_CENTER.z};
        /// @brief Draw the goldfish inside the aquarium: a diamond cube body, a pyramid tail and
        ///        two eyes, head pointing -Z.
        ...
        /**
         * @brief Draw the windshield on the body's raked front-top.
         *
         * That rake is the hull edge skipped in drawBodyShell; here it becomes a body-colour
         * border around an opening, with one semi-transparent glass quad recessed inside it
         * and reveal walls closing the depth.
         */
        ...
        /// @brief Draw the grey headlight frames flanking the emblem (lit with the body); the
        ///        emissive lens cores are drawn separately by drawHeadlightLenses.
        ...
        /// @brief Draw the hot headlight lens cores, drawn emissive by the caller (halo in drawCarGlow).
        ...
        /// @brief Draw the two grey roof cross-bars.
        void drawRoofBars(bool wire) { drawParts(ROOFBAR_PARTS, wire); }

        /// @brief Place the four road wheels via matrix transforms; geometry from drawWheel.
        ...
        /// @brief Draw the two red rectangle tail-lights above the rear bumper (emissive cores).
        ...
        /// @brief Draw one additive radial glow sprite on a Z-facing plane (culling is off, so
        ///        winding is irrelevant -- it just blooms outward).
        ...
        /**
         * @brief Draw the shared "yellow disk + three orange crossing sticks" badge.
         *
         * Modelled once in a canonical frame (disk axis +Y, sticks in the XZ plane rotated
         * about Y); the front emblem and each wheel hub orient it with a wrapping matrix.
         * @param radius         Disk radius.
         * @param depth          Disk thickness along its axis.
         * @param sides          Disk facet count.
         * @param stickLenFactor Stick length as a fraction of the disk diameter.
         * @param stickW         Stick width.
         * @param stickH         Stick height.
         * @param proud          Offset of the sticks above the disk face, along +Y.
         */
        ...
        /// @brief Draw the front emblem on the bumper: the shared badge rotated to face forward (-Z).
        ...
        /// @brief Draw the side decal (the only textured geometry), mirrored across the sides.
        /// @param sx Side: -1 = left, +1 = right.
        ...
        /// @brief Draw the dark asphalt ground patch the van rests on, giving the headlight
        ///        spotlights a surface to pool on.
        ...
        /// @brief Draw the turquoise side mirrors on both sides: a short arm plus an upright head.
    ...
    /// @brief Draw the chassis: the extruded-hull body (with grey bumpers) plus the roof bars.
    void drawChassis(bool wire) {
        drawBodyShell(wire);
        drawRoofBars(wire);
    }

    /// @brief Draw one road wheel at the local origin (axle along X): a black tyre with the
    ///        shared emblem badge on each outer face.
    ...
    /// @brief Load the van's textures (idempotent).
    ...
    /// @brief Draw the opaque, scene-lit parts: ground, chassis, seats, mirrors, fish, wheels,
    ///        emblem, headlight housings and side decals.
    ...
    /// @brief Draw the emissive light cores (headlight lenses + tail-lights); the caller draws
    ...
    /// @brief Draw the additive bloom halos around each light -- a glow effect (not a surface
    ///        material), drawn outside the lighting shader with additive blending + depth-write off.
    ...
    /// @brief Draw the transparent parts. The aquarium goes first: it sits inside the cabin, so
    ///        the windows must blend over it (drawing it after would let their depth occlude it).
    ...
    /// @brief Draw the whole van in one pass (body, light cores, glass, glow).
